        ...
        if len(rectangles) == 1:  # 限制一张人脸
            for rectangle in rectangles:
                ...
                for erx0, ery0, erx1, ery1 in rectangles_eye:
                    eye_flow = opt_flow(prev_gray[ery0:ery1, erx0:erx1], gray[ery0:ery1, erx0:erx1])  # get opt flow
                    lines = draw_flow(img[ry0:ry1, rx0:rx1][ery0:ery1, erx0:erx1], eye_flow, step=4)  # 显示光流点
                    eye_flow_lines.append(lines)

                    # mag, ang = cv2.cartToPolar(eye_flow[..., 0], eye_flow[..., 1])
                    # eye_flow_angs.append(ang)

                    # for a in ang:
                    #     print str(a*180/np.pi/2) + ','
                    # print ''
